...
>  [查看钩子函数](https://cn.vuejs.org/v2/guide/custom-directive.html#%E9%92%A9%E5%AD%90%E5%87%BD%E6%95%B0)
...
和局部注册组件一样、为了让指令更加通用、通常会吧指令的配置提取到其他模块。
...
`vue`支持在指令中配置一些**钩子函数**、在适当的饿机会、`vue`会调用这些钩子函数、并传入适当的参数、以便开发者完成自己想做的事情。
